...
\mathbf{f_{qr}} (\mathbf{p},\mathbf{a'}) = 
 \left(
\begin{array}{c}
 a'_x + 2 \left(-(q_y^2+ q_z^2) a'_x +(q_x q_y - q_r q_z) a'_y+(q_r q_y+q_x q_z) a'_z \right)  \\
 a'_y + 2 \left((q_r q_z+  q_x q_y) a'_x-(q_x^2 +q_z^2) a'_y+(q_y q_z-q_r q_x) a'_z \right) \\
 a'_z + 2 \left((q_x q_z-  q_r q_y) a'_x+(q_r q_x + q_y q_z) a'_y-(q_x^2+q_y^2) a'_z \right)  \\
\end{array}
\right)
