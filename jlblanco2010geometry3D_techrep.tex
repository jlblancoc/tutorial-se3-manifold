...
\begin{abstract}
Any rigid transformation in 3D Euclidean space, or \emph{pose}, comprises a 3D translation and an arbitrary 3D rotation.
...
The text also reflects which MRPT C++ library function implements 
...
\begin{eqnarray}
\mathbf{p_7} &=& [x ~ y ~ z ~  q_r ~ q_x ~ q_y ~ qz ] ^ \top
\end{eqnarray}

\noindent where the quaternion elements are $[q_r, (q_x,q_y,qz)]$. A useful interpretation of quaternions
is that of a rotation of $\theta$ radians around the axis defined by the vector $\vec{v} = (v_x,v_y,v_z) \propto (q_x,q_y,qz)$.
...
\begin{eqnarray}
\mathbf{R}(\phi,\chi,\psi) &=&  \mathbf{R}_z(\phi) \mathbf{R}_y(\chi) \mathbf{R}_x(\psi) \label{eq:mat_ypr} \\
&=& 
\left(
\begin{array}{ccc}
